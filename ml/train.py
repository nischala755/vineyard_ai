...
def main() -> None:
    ...
    model=tf.keras.Sequential([augment(),tf.keras.applications.mobilenet_v3.preprocess_input,base,tf.keras.layers.GlobalAveragePooling2D(),tf.keras.layers.Dropout(.25),tf.keras.layers.Dense(4,activation='softmax',dtype='float32')])
    ...
    converter=tf.lite.TFLiteConverter.from_keras_model(model); converter.optimizations=[tf.lite.Optimize.DEFAULT]; converter.representative_dataset=lambda: ((x,) for x,_ in train.take(100)); converter.target_spec.supported_ops=[tf.lite.OpsSet.TFLITE_BUILTINS_INT8]; converter.inference_input_type=tf.uint8; converter.inference_output_type=tf.uint8; (a.output/'grape_disease_int8.tflite').write_bytes(converter.convert()); (a.output/'labels.txt').write_text('\n'.join(LABELS)+'\n')
